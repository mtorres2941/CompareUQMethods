...
Added after the author asked how to fix Silverman's degenerate-IQR
...
NOTHING HERE IS ADOPTED. The bandwidth robustness sweep is still queued.

    conda run -n compareuq python audits/bandwidth_rules.py [n_synth]
"""
import os
import sys
import warnings
...
def analyse_guarded():
    ...
    """
    d = pd.read_csv(os.path.join(TABLES, 'TABLE_BandwidthRules.csv'))
    ...
    r = pd.DataFrame(rows)
    r.to_csv(os.path.join(TABLES, 'TABLE_GuardedBandwidthSweep.csv'),
             index=False)
    print()
    print('=' * 78)
...
def main(n_synth):
    ...
    d = pd.DataFrame(rows)
    d.to_csv(os.path.join(TABLES, 'TABLE_BandwidthRules.csv'), index=False)
