...
#: The values the generator configuration was originally set from, for
...
def main():
    ...
    met.to_csv(os.path.join(TABLES, 'TABLE_EmpiricalEnvelope.csv'))
    ...
    print(f'  median      {cv.median():.4f}   (previous arm {OLD["coeffvar_median"]})')
    print(f'  log10 sd    {lcv.std():.4f}   (previous arm {OLD["coeffvar_log10_sd"]})')
    print(f'  log10 mean  {lcv.mean():.4f}   median log10 {lcv.median():.4f}')
    print(f'  min / max   {cv.min():.4f} / {cv.max():.4f}   '
          f'(previous arm {OLD["coeffvar_min"]} / {OLD["coeffvar_max"]})')
    ...
    print(f'  median      {sk.median():.4f}   (previous arm {OLD["skewness_median"]})')
    print(f'  min / max   {sk.min():.4f} / {sk.max():.4f}   '
          f'(previous arm {OLD["skewness_min"]} / {OLD["skewness_max"]})')
    ...
    print('\n=== dataset size ===')
    n = met["n"]
    print(f'  median {n.median():.0f}   (previous arm {OLD["n_median"]})   '
    ...
    print(f'  unimodal    {(modes == 1).mean()*100:5.1f}%   '
          f'(previous arm {OLD["unimodal_share"]*100:.1f}%)')
    ...
    pd.DataFrame(dict(material=met.index, n_modes=modes)).to_csv(
        os.path.join(TABLES, 'TABLE_EmpiricalModality.csv'), index=False)
