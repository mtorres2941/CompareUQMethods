...
Stage 2b. `families.PROFILE_DELTA_LO_FRAC` was set to 0.01 on the reasoning that
...
matter together: the largest standard deviation of any fitted model, and W1.
    ...
    conda run -n compareuq python audits/stage2b/r7_profile_guard.py [n_synth]
"""
import os
import sys
import warnings
...
def main(n_synth):
    ...
    d = pd.DataFrame(rows)
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_ProfileGuardSweep.csv'), index=False)
