...
Stage 2b, added after the author asked. The stage's headline finding is that the
...
estimation.
  ...
  3. THE CRITERION. W1 falls monotonically as the bandwidth shrinks, all the way
     ...
     penalty" and it makes Stage 2c's out-of-sample comparison decisive rather
     than optional.

NOTHING HERE IS ADOPTED. The bandwidth rule belongs to Stage 2h and the
criterion to Stage 2c. This script measures; it changes no default.

    conda run -n compareuq python audits/stage2b/r8_why_kde_loses.py
"""
import os
import sys
import warnings
...
def mechanism_1():
    ...
    pd.concat(out).to_csv(os.path.join(TABLES, 'TABLE_2b_RankBySize.csv'),
                          index=False)
    ...
    comp.to_csv(os.path.join(TABLES, 'TABLE_2b_PostStratifiedRank.csv'))
...
def mechanism_2_and_3(ds):
    ...
    for name, (x, w) in ds.items():
        ...
        rows.append(r)
    d = pd.DataFrame(rows)
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_BandwidthDiagnostic.csv'),
             index=False)
    ...
    for name, (x, w) in ds.items():
        ...
        rows.append(r)
    d = pd.DataFrame(rows)
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_BandwidthSweepW1.csv'), index=False)
    ...
    print('though the criterion is biased in the KDE\'s favour. Stage 2c owns the')
    print('out-of-sample answer; Stage 2h owns the bandwidth.')
