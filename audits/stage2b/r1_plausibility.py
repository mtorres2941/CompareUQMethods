...
Stage 2b, task 1. The author's decision of 2026-09-14: remove records that
...
4. how many records the extraction excludes for having a non-positive GWP. A
   count for the manuscript, not a change
    ...
    conda run -n compareuq python audits/stage2b/r1_plausibility.py
"""
import os
import sys
...
def nonpositive_gwp():
    ...
    try:
        import p1_build_raw_extract as p1
...
def main():
    ...
    dropped.to_csv(os.path.join(TABLES, 'TABLE_2b_PlausibilityDropped.csv'))
    ...
    eff.to_csv(os.path.join(TABLES, 'TABLE_2b_PlausibilityEffect.csv'),
               index=False)
    print()
    ...
    print('=' * 72)
    ext = extremes(arm)
    ext.to_csv(os.path.join(TABLES, 'TABLE_2b_UnitExtremes.csv'), index=False)
    ...
    npg = nonpositive_gwp()
    if npg is not None:
        npg.to_csv(os.path.join(TABLES, 'TABLE_2b_NonPositiveGWP.csv'),
                   index=False)
