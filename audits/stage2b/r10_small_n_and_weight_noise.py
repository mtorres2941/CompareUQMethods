...
Stage 2b, added after the author refused two results on sniff-test grounds. Both
...
measures it by drawing the weights twice on the same values.
    ...
    conda run -n compareuq python audits/stage2b/r10_small_n_and_weight_noise.py
"""
import os
import sys
import warnings
...
def q2_weight_noise():
    ...
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_WeightTargetNoise.csv'), index=False)
...
def q2b_rank_stability(k=N_REALIZATIONS):
    ...
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_RankStability.csv'), index=False)
