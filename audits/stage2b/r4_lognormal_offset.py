...
Stage 2b. Three questions, in order.
...
Near-zero values are far rarer than they were. Stage 1 measured 28.3 percent of
138 empirical datasets with a minimum below 1 percent of their mean; the
symmetric log-space cleaning of Stage 2a-2 and the plausibility ceiling of
Stage 2b bring that to the figure this script prints. They have not gone away.

    conda run -n compareuq python audits/stage2b/r4_lognormal_offset.py
"""
import os
import sys
import warnings
...
#: fraction of the mean by construction. 1 percent is the threshold Stage 1
...
def main():
    ...
    d.to_csv(os.path.join(TABLES, 'TABLE_2b_OffsetDiagnosis.csv'), index=False)
    ...
    print(f'Stage 1 measured 28.3 percent of 138. The symmetric log-space '
    ...
    p.to_csv(os.path.join(TABLES, 'TABLE_2b_ThresholdPathology.csv'),
             index=False)
