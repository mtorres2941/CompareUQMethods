"""FROZEN COPY of src/datageneration.py as it stood at commit 7cf9964.

This is the generator that produced the shipped data/processed/DATA_all.json.
It is kept verbatim, byte for byte from git, so that an earlier revision can measure the
seeding collapse it caused rather than arguing about it from the diff. Do not
edit it and do not import it from src/ or from any notebook.

Retrieved with:  git show 7cf9964:src/datageneration.py

The two defects it carries, both fixed in the original code commit 533f6c4:
  - generate_random_numbers(..., seed=0) builds a fresh Generator on every
    call, so component draws are a deterministic function of (type, count) and
    the shape parameters are constants across the entire study.
  - the lognorm branch omits random_state, so it alone draws from global numpy
    state.
"""
