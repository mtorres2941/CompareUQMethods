...
an earlier revision, after the declared-unit axis was withdrawn. The question is not
...
worth keeping: the refutation is the more useful result.
    ...
    conda run -n compareuq python audits/split_axis_evidence.py
"""
import os
import sys
...
def main():
    ...
    tab.to_csv(os.path.join(TABLES, 'TABLE_SplitAxisEvidence.csv'),
               index=False)
    ...
    print(f"\nwrote {os.path.relpath(os.path.join(TABLES, 'TABLE_SplitAxisEvidence.csv'), ROOT)}")
