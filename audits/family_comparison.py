...
an earlier revision. Four questions that share one loop, so they share one script.
...
Q2. WHAT THE ORIGINAL SCORING WAS ACTUALLY DOING. The old grid started at
...
Q4. FITTING BY THE CRITERION WE SCORE BY. Every parametric family under both
    ...
    conda run -n compareuq python audits/family_comparison.py [n_synth]
"""
import os
import sys
import time
...
def one_dataset(name, x, w_var, arm):
    ...
    for wt, w in (('Uniform', FT.uniform_weights(x)), ('Variable', w_var)):
        ...
        # bandwidth, which is an earlier revision's.
...
def truncation_rows(name, x, w_var, arm):
    ...
    `w1_original` is the number the fitting produced as originally written,
    kept in the same row, because the second
    ...
    for wt, w in (('Uniform', FT.uniform_weights(x)), ('Variable', w_var)):
        ...
        out.append(dict(
            ...
            w1_original=FT.score_w1(parent, x, w_var)))
...
def main(n_synth, redo_truncation=True):
    ...
    tpath = os.path.join(TABLES, 'TABLE_TruncationEffect.csv')
    ...
    d = pd.DataFrame(rows)
    d.to_csv(os.path.join(TABLES, 'TABLE_FamilyComparison.csv'), index=False)
    if redo_truncation:
    ...
    print('WHAT THE ORIGINAL SCORING WAS DOING, normal only')
    n = t[t.family == 'normal'].dropna(subset=['w1_original'])
    n = n.assign(vs_trunc=(n.w1_original - n.w1_truncated).abs() / n.w1_truncated,
                 vs_untrunc=(n.w1_original - n.w1_untruncated).abs()
                 / n.w1_untruncated)
    print(f'  the original code W1 against the TRUNCATED score:   median relative '
          f'difference {n.vs_trunc.median()*100:.3f} pct')
    print(f'  the original code W1 against the UNTRUNCATED score: median relative '
          f'difference {n.vs_untrunc.median()*100:.3f} pct')
    print('  The old grid started at exactly zero, so the original code was already')
