...
- **Pull fresh EC3 data.** The analysis currently uses a stored 2026-03 pull
  that was already trimmed at the high end before it was saved, which is the
  only reason Stage 2a applied a multiplicative bound to the low end alone. The
  API key and procedure are in `../EPDsFromEC3`. With raw values in hand the
  symmetric log-space rule can be applied, which the sensitivity analysis
  prefers: the choice of rule moves `fit_norm_SW` by about 1.5 standard
  deviations. **An earlier version of this handoff said the extraction could
  not be reproduced; that was wrong, and the snapshot question it raised is
  irrelevant** - the analysis is being redone, so current data is what is
  wanted. Re-pulling moves every empirical number, which is expected.
  Discrepancy entry 26.
...
- **The coefficient of variation sits below the empirical median**, 0.487
  against 0.593 after tuning, W1 0.465. The `Q3 + 3*IQR` truncation caps how
  much right tail survives. **Owner: 2h**, as a sweep of `trunc_iqr_mult`.
...
`../EPDsFromEC3/store/`. `../EPDsFromEC3/PULLING_EPDS.md` documents how to take
a fresh EC3 pull and the three ways a paginated pull fails silently; read it
before writing anything that talks to that API.
...
| **Take a fresh EC3 pull** and clean symmetrically | The stored file is a 2026-03 pull, saved post-cleaning, so Stage 2a could only trim the low end. Current data is what the redone analysis wants. Moves every empirical number, so it needs a deliberate go-ahead. `../EPDsFromEC3` has the API key and `PULLING_EPDS.md`. Discrepancy entry 26 |
