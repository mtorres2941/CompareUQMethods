...
- **The stored empirical file is post-cleaning, and a fresh pull is
  available.** `dct_realeccs_trimmed.json` was already trimmed additively at
  the high end when written, which is why Stage 2a applied only a
  multiplicative LOW-end bound. That is a limitation of the stored file, not of
  the data source: the EC3 API key and a documented procedure are in
  `../EPDsFromEC3`, so a fresh pull can be taken whenever wanted and the
  symmetric log-space rule applied to raw values. **An earlier version of this
  handoff said the extraction could not be reproduced; that was wrong.** The
  low-end-only decision rested on it and should be revisited: the sensitivity
  analysis says the choice of cleaning rule moves `fit_norm_SW` by about 1.5
  standard deviations. Discrepancy entry 26.
...
| Whether to re-pull from EC3 and clean symmetrically | The stored empirical file is post-cleaning, so Stage 2a could only trim the low end. A fresh pull is available at any time (`../EPDsFromEC3`, API key and procedure), and would allow the symmetric rule. Not doing it is a choice, not a constraint. Discrepancy entry 26 |
