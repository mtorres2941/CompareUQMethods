...
| **SUPERSEDED by Stage 2a-3** | The arm is now **142 datasets drawn from 136 categories**: six categories hold more than one product population and are split into twelve. 142 is the number every per-dataset statement should use; 136 is the number of EC3 categories the arm was drawn from. Both belong in the text, and the split table (`outputs/tables/TABLE_EmpiricalCategorySplit.csv`) is what makes 142 derivable. |
| **Status** | Open. Text edit, but it appears in many places. The number to write is 142 datasets from 136 categories. |
